...
function loadNotepad(container) {
    ...
    container.innerHTML = `<textarea class="notepad-textarea" id="notepad-text" placeholder="写点什么...">${escapeHtml(saved)}</textarea><div style="margin-top:8px;"><button class="system-popup-btn" id="save-notepad">保存</button></div>`;
    ...
    container.querySelector('#save-notepad').addEventListener('click', () => {
        ...
        playSound('click');
        setTimeout(() => { showHeeseungPopup('你在写什么。', [{ text:'我在记录', reply:'我也在记录你。', residue:1 }]); }, 3000);
    });
}
...
function loadCalculator(container) {
    ...
    const calc = () => {
        ...
        update();
        if(result === 5254 || display === '5254') showHeeseungPopup('你还记得这个数字。', [{ text:'...', reply:'你以前告诉我的。', residue:2 }]);
...
function loadAlarm(container) {
    ...
    container.querySelector('.alarm-edit')?.addEventListener('click', () => { showHeeseungPopup('你想改这个时间吗？', [{ text:'不改了', reply:'嗯。', residue:0 }]); });
}
...
function loadContacts(container) {
    ...
    container.querySelector('#heeseung-contact').addEventListener('click', () => { showHeeseungPopup('你想联系我吗。', [{ text:'你在吗', reply:'我一直在。', residue:2 },{ text:'我想起来了', reply:'真的吗。', residue:3 }]); });
}
...
function loadRun(container) {
    ...
    container.querySelector('#run-ok').addEventListener('click', () => {
        const cmd = input.value.toLowerCase();
        if(cmd === 'heeseung' || cmd === 'hee') showHeeseungPopup('你在叫我吗。', [{ text:'嗯', reply:'我在。', residue:2 }]);
